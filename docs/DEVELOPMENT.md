...
Replays `tests/session-sounds/fprintd-trace.txt` - `fprintd`'s signals, recorded
during real `sudo` and `pkexec` prompts - through the rules in
`src/session_sounds.py`, and checks the timeout parsing against
